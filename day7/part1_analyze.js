...
function findTopDirectories(currentDirTree, max = 100000, dirs = []) {
    Object.keys(currentDirTree.dirs).forEach((d) => {
        findTopDirectories(currentDirTree.dirs[d], max, dirs);
    });

    if (currentDirTree.size <= max) {
        dirs.push({name: currentDirTree.name, size: currentDirTree.size});
    }
...
let dirs = findTopDirectories(tree['/']);
